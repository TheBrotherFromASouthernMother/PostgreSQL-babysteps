...
CREATE TABLE techPerProjects (project_name TEXT, num_tech integer);
INSERT INTO techPerProjects SELECT name, COUNT(project_uses_tech.project_id) AS num_tech FROM project LEFT OUTER JOIN project_uses_tech ON project.id = project_uses_tech.project_id GROUP BY name ORDER BY num_tech;
SELECT AVG(num_tech) FROM techPerProjects;
